...
def validate_user_args():
    if _exec_mode_unset():
        _set_default_exec_mode()
    elif _probabilistic_mode_without_strategy():
        _set_default_strategy()
    
    if _strategy_set_with_non_probabilistic_mode() or _use_cache_with_architecture_unset():
        print("Error: enter the \"-h\" parameter on the command line after \"python script.py\" to see usage instructions")
        sys.exit()

def _exec_mode_unset():
    return Constantes().g_argsp_exec_mode is None

def _set_default_exec_mode():
    Constantes().g_argsp_exec_mode = 'probabilistic' if Constantes().g_argsp_strategy else 'manual'

def _probabilistic_mode_without_strategy():
    return Constantes().g_argsp_exec_mode == 'probabilistic' and Constantes().g_argsp_strategy is None

def _set_default_strategy():
    Constantes().g_argsp_strategy = 'error'

def _strategy_set_with_non_probabilistic_mode():
    return Constantes().g_argsp_strategy is not None and Constantes().g_argsp_exec_mode != 'probabilistic'

def _use_cache_with_architecture_unset():
    return Constantes().g_argsp_exec_mode != 'no-cache' and Constantes().g_argsp_m is None
...
if Constantes().g_argsp_exec_mode == 'no-cache':
    ...
    def _start_function_inference_engine(experiment:Experiment) -> None:
        ...
        command = f"python {sys.argv[0]} {' '.join(Constantes().g_argsp_exp_args)} --exec-mode=no-cache"
